...
private slots:
    void onServerStarted()
    {
        ...
        QMessageBox::information(this, "成功", "服务器启动成功！应用已准备就绪。");
    }
...
private:
    void setupUI()
    {
        ...
        // 创建各个页面
        SingleFileConverter *singleConverter = new SingleFileConverter(m_httpApi, this);
        MultiFileConverter *multiConverter = new MultiFileConverter(m_httpApi, this);
    ...
    void startEmbeddedServer()
    {
        if (!m_embeddedServer) {
            ...
            connect(m_embeddedServer, &EmbeddedServer::serverStarted, 
                    this, &SimpleIntegratedMainWindow::onServerStarted);
            connect(m_embeddedServer, &EmbeddedServer::serverStopped, 
                    this, &SimpleIntegratedMainWindow::onServerStopped);
            connect(m_embeddedServer, &EmbeddedServer::serverError, 
                    this, &SimpleIntegratedMainWindow::onServerError);
            connect(m_embeddedServer, &EmbeddedServer::healthCheckResult, 
                    this, &SimpleIntegratedMainWindow::onServerHealthChanged);
        }
...
QSplashScreen* showSplashScreen()
{
    ...
    painter.drawText(pixmap.rect(), Qt::AlignCenter, "Markdown转Word工具\n整合版");
