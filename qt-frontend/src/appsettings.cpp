...
const QString AppSettings::KEY_LAST_TEMPLATE_DIR = "directories/lastTemplateDir";
...
{
    return m_settings->value(KEY_LAST_INPUT_DIR, 
...
{
    return m_settings->value(KEY_LAST_OUTPUT_DIR, 
        QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation)).toString();
}
...
{
    return m_settings->value(KEY_LAST_TEMPLATE_DIR, 
        QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation)).toString();
}
...
{
    m_settings->remove(KEY_RECENT_FILES);
}
