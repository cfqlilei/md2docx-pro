...
void HttpApi::setServerUrl(const QString &url)
{
    m_serverUrl = url;
}
...
void HttpApi::checkHealth()
{
    ...
    connect(reply, &QNetworkReply::finished, this, &HttpApi::onHealthCheckFinished);
...
void HttpApi::updateConfig(const ConfigData &config)
{
    ...
    connect(reply, &QNetworkReply::finished, this, &HttpApi::onUpdateConfigFinished);
    connect(reply, QOverload<QNetworkReply::NetworkError>::of(&QNetworkReply::error),
            this, &HttpApi::onNetworkError);
}
...
void HttpApi::validateConfig()
{
    ...
    connect(reply, &QNetworkReply::finished, this, &HttpApi::onValidateConfigFinished);
    connect(reply, QOverload<QNetworkReply::NetworkError>::of(&QNetworkReply::error),
            this, &HttpApi::onNetworkError);
}
...
void HttpApi::convertSingle(const ConversionRequest &request)
{
    ...
    connect(reply, &QNetworkReply::finished, this, &HttpApi::onSingleConversionFinished);
    connect(reply, QOverload<QNetworkReply::NetworkError>::of(&QNetworkReply::error),
            this, &HttpApi::onNetworkError);
}
...
void HttpApi::convertBatch(const BatchConversionRequest &request)
{
    ...
    connect(reply, &QNetworkReply::finished, this, &HttpApi::onBatchConversionFinished);
    connect(reply, QOverload<QNetworkReply::NetworkError>::of(&QNetworkReply::error),
            this, &HttpApi::onNetworkError);
}
...
void HttpApi::handleNetworkReply(QNetworkReply *reply, const QString &operation)
{
    if (reply->error() != QNetworkReply::NoError) {
        QString errorMsg = QString("网络错误 (%1): %2").arg(operation).arg(reply->errorString());
    ...
    if (parseError.error != QJsonParseError::NoError) {
        QString errorMsg = QString("JSON解析错误 (%1): %2").arg(operation).arg(parseError.errorString());
        emit errorOccurred(errorMsg);
        return;
    }
}
